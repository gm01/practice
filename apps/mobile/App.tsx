...
function mobileHasFilters(filters:PlayerSearchFilters){return Object.entries(filters).some(([key,value])=>!["query","grade","sort","limit"].includes(key)&&(Array.isArray(value)?value.length:value!==undefined&&value!==""))}
...
function MobilePlayerFilters({value,meta,onChange,onReset}:{value:PlayerSearchFilters;meta:PlayerFilterMetadata|null;onChange:(value:PlayerSearchFilters)=>void;onReset:()=>void}){
  ...
  return <View style={s.mobileFilterPanel}><View style={s.mobileFilterHeading}><View><Text style={s.headingCompact}>조건 검색</Text><Text style={s.muted}>여러 조건을 함께 선택할 수 있습니다.</Text></View><Pressable onPress={onReset}><Text style={s.green}>전체 초기화</Text></Pressable></View>
    <Text style={s.mobileFilterTitle}>시즌</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.mobileFilterChips}>{meta?.seasons.map(row=><Pressable style={[s.mobileSeasonChip,value.seasonIds?.includes(row.id)&&s.mobileFilterChipActive]} onPress={()=>onChange({...value,seasonIds:mobileToggle(value.seasonIds,row.id)})} key={row.id}>{row.imageUrl&&<Image source={{uri:row.imageUrl}} style={s.mobileSeasonImage}/>}<Text numberOfLines={1} style={value.seasonIds?.includes(row.id)?s.mobileFilterChipTextActive:s.mobileFilterChipText}>{row.name}</Text></Pressable>)}</ScrollView>
