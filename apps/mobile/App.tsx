...
function MobilePlayerFilters({value,meta,onChange,onReset,onApply}:{value:PlayerSearchFilters;meta:PlayerFilterMetadata|null;onChange:(value:PlayerSearchFilters)=>void;onReset:()=>void;onApply:()=>void}){
  ...
  return <View style={s.mobileFilterPanel}><View style={s.mobileFilterHeading}><View><Text style={s.headingCompact}>조건 검색</Text><Text style={s.muted}>여러 조건을 함께 선택할 수 있습니다.</Text></View><Pressable onPress={onReset}><Text style={s.green}>전체 초기화</Text></Pressable></View>
    <Text style={s.mobileFilterTitle}>팀컬러</Text><View style={s.mobileTeamColorRow}><Pressable style={[s.mobileTeamColorButton,selectedTeamColor&&s.mobileTeamColorButtonActive]} onPress={()=>setTeamColorOpen(true)} accessibilityRole="button" accessibilityLabel="팀컬러 검색 열기"><Text numberOfLines={1} style={selectedTeamColor?s.mobileFilterChipTextActive:s.mobileFilterChipText}>{selectedTeamColor?.name??"팀컬러 검색·선택"}</Text>{selectedTeamColor&&<Text style={s.mobileTeamColorLevel}>{selectedTeamColor.level}단계</Text>}</Pressable>{selectedTeamColor&&<Pressable style={s.mobileTeamColorClear} onPress={()=>onChange({...value,teamColorId:undefined})} accessibilityLabel="선택한 팀컬러 해제"><Text style={s.green}>해제</Text></Pressable>}</View>
