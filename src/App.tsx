...
function Lineup({ players, onSelectPlayer }: { players: PlayerSummary[]; onSelectPlayer: (spId: number) => void }) {
  return <div className="formation-pitch">{players.filter((player) => player.positionCode < 28 && player.rating > 0).map((player) => {
    ...
    return <button className="formation-player" onClick={()=>onSelectPlayer(player.spId)} style={{ left: `${x}%`, top: `${y}%` }} key={player.spId}>
...
function MatchDetail({ match, myNickname, onBack, onSelectPlayer }: { match: MatchSummary; myNickname: string; onBack: () => void; onSelectPlayer: (spId: number, side: "mine" | "opponent") => void }) {
  ...
  const myRepresentative = match.players.reduce<PlayerSummary | null>((best, player) => player.rating > (best?.rating ?? 0) ? player : best, null);
  const opponentRepresentative = match.opponentPlayers.reduce<PlayerSummary | null>((best, player) => player.rating > (best?.rating ?? 0) ? player : best, null);
  return <section className="match-page">
    ...
    <div className="match-hero">
      ...
      <div className="scoreboard">
        <div className="club"><div className="club-photo">{myRepresentative?<PlayerPhoto player={myRepresentative}/>:<span>{myNickname.slice(0,1)}</span>}</div><h2>{myNickname}</h2><small>{match.divisionName}</small></div>
        <div className="final-score"><strong>{match.myScore}<i>:</i>{match.opponentScore}</strong><span>{endLabel}</span></div>
        <div className="club away"><div className="club-photo">{opponentRepresentative?<PlayerPhoto player={opponentRepresentative}/>:<span>{match.opponentNickname.slice(0,1)}</span>}</div><h2>{match.opponentNickname}</h2><small>{match.opponentDivisionName}</small></div>
      </div>
      <div className="scorer-summary"><div>{match.goals.filter(g=>g.side==="mine").map((goal,i)=><span key={i}><b>{goal.playerName} {goal.minute}'</b>{goal.assistName&&<small>도움 · {goal.assistName}</small>}</span>)}{match.ownGoalsFor>0&&<span><b>상대 자책골 ×{match.ownGoalsFor}</b><small>발생 시간 정보 없음</small></span>}</div><div>{match.goals.filter(g=>g.side==="opponent").map((goal,i)=><span key={i}><b>{goal.minute}' {goal.playerName}</b>{goal.assistName&&<small>도움 · {goal.assistName}</small>}</span>)}{match.ownGoalsAgainst>0&&<span><b>내 자책골 ×{match.ownGoalsAgainst}</b><small>발생 시간 정보 없음</small></span>}</div></div>
    </div>
    ...
    {tab === "summary" && <div className="detail-section">
      ...
      <h3>득점 타임라인</h3><div className="event-list">{match.goals.map((goal,index)=><div className={`event ${goal.side}`} key={index}>{goal.side==="mine"?<><b className="event-minute">{goal.minute}'</b><span className="event-scorer"><strong>⚽ {goal.playerName}</strong>{goal.assistName&&<em>도움 · {goal.assistName}</em>}</span><i/><i/></>:<><i/><i/><span className="event-scorer away">{goal.assistName&&<em>도움 · {goal.assistName}</em>}<strong>{goal.playerName} ⚽</strong></span><b className="event-minute">{goal.minute}'</b></>}</div>)}{Array.from({length:match.ownGoalsFor},(_,index)=><div className="event mine own-goal" key={`own-for-${index}`}><b className="event-minute">–</b><span className="event-scorer"><strong>⚽ 상대 자책골</strong><em>선수·시간 정보 없음</em></span><i/><i/></div>)}{Array.from({length:match.ownGoalsAgainst},(_,index)=><div className="event opponent own-goal" key={`own-against-${index}`}><i/><i/><span className="event-scorer away"><em>선수·시간 정보 없음</em><strong>내 자책골 ⚽</strong></span><b className="event-minute">–</b></div>)}{match.goals.length===0&&match.ownGoalsFor===0&&match.ownGoalsAgainst===0&&<p>득점 기록이 없습니다.</p>}</div>
      <h3>최고 평점 선수</h3><div className="top-player-grid">{match.topPlayers.map(player=><button className="top-player" onClick={()=>onSelectPlayer(player.spId,"mine")} key={player.spId}><PlayerPhoto player={player}/><div><b>{player.name}</b><span>{player.position} · +{player.grade}강</span><small>{player.goals}골 · {player.assists}도움</small></div><strong>{player.rating.toFixed(1)}</strong></button>)}</div>
    </div>}
...
export default function App() {
  ...
  const [selectedPlayer,setSelectedPlayer]=useState<{spId:number;side:"mine"|"opponent"}|null>(null);
  const [loading,setLoading]=useState(false); const [error,setError]=useState(""); const [hasMore,setHasMore]=useState(false); const [nextOffset,setNextOffset]=useState(0); const requestInFlight=useRef(false);
  ...
  return <main className="shell"><header className="topbar"><div className="brand"><img className="brand-mark" src={appIcon} alt="FC Online Lab"/><span>ONLINE LAB</span></div><button className="login-link" onClick={()=>void window.fcOnline.openLogin()}>넥슨 로그인 ↗</button></header>
    {selectedPlayer ? <PlayerDetail matches={matches} spId={selectedPlayer.spId} side={selectedPlayer.side} onBack={()=>setSelectedPlayer(null)} onOpenMatch={(matchId)=>{setSelectedPlayer(null);setSelectedId(matchId);}}/> : selectedMatch ? <MatchDetail match={selectedMatch} myNickname={profile?.nickname??nickname} onBack={()=>setSelectedId(null)} onSelectPlayer={(spId,side)=>setSelectedPlayer({spId,side})}/> : <>
