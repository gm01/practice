...
function PlayerSearchFiltersPanel({ value, meta, onChange, onReset }: { value: PlayerSearchFilters; meta: PlayerFilterMetadata|null; onChange:(value:PlayerSearchFilters)=>void; onReset:()=>void }) {
...
export default function PlayerDatabase({ matches, onHeaderBackChange, initialQuery = "" }: { matches: MatchSummary[]; onHeaderBackChange?: (handler: (() => void) | null) => void; initialQuery?: string }) {
  ...
  if(selected)return <section className="player-db">
    ...
    {detail&&<>
      ...
      <div className="db-training-heading"><div><h2>세부 능력치 · 집중훈련</h2><small>능력치별 최대 +2 · 현재 강화 단계는 {trainingLimit}개까지 선택 가능</small></div><div><b>{trainedCount}/{trainingLimit}</b><button onClick={()=>setFocusedTraining({})} disabled={trainedCount===0}>초기화</button></div></div><div className="db-abilities">{detail.abilities.map(row=>{const training=focusedTraining[row.label]??0,totalDelta=row.delta+training,value=row.value+training,canAdd=training<2&&(training>0||trainedCount<trainingLimit);return <span className={totalDelta>0?"boosted":""} key={row.label}><small>{row.label}</small><b style={{color:statColor(value)}}>{value}{totalDelta>0&&<em>+{totalDelta}</em>}</b><i className="db-training-controls"><button onClick={()=>changeFocusedTraining(row.label,-1)} disabled={training===0} aria-label={`${row.label} 집중훈련 감소`}>−</button><strong>{training}</strong><button onClick={()=>changeFocusedTraining(row.label,1)} disabled={!canAdd} aria-label={`${row.label} 집중훈련 증가`}>＋</button></i></span>})}</div>
