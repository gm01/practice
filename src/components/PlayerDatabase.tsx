...
function hasFilters(filters: PlayerSearchFilters) { return Object.entries(filters).some(([key,value])=>!['query','grade','sort','limit','page','pageSize'].includes(key)&&(Array.isArray(value)?value.length:value!==undefined&&value!=="")); }
...
function PlayerComparison({ cards, onClose }: { cards: [PlayerCard,PlayerCard]; onClose: () => void }) {
  ...
  useEffect(()=>{let active=true;setLoading(true);setLoadedComparison(null);setError("");Promise.all(cards.map((card,index)=>window.fcOnline.fetchPlayerDetail(card.spId,grades[index],{adaptation:1}))).then(values=>{if(active)setLoadedComparison({key:comparisonKey,values:values as [PlayerDetail,PlayerDetail]})}).catch(reason=>{if(active)setError(reason instanceof Error?reason.message:"선수 비교 정보를 불러오지 못했습니다.")}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[cards,grades]);
...
export default function PlayerDatabase({ matches, onHeaderBackChange, initialQuery = "" }: { matches: MatchSummary[]; onHeaderBackChange?: (handler: (() => void) | null) => void; initialQuery?: string }) {
  ...
  const [loadedDetail,setLoadedDetail]=useState<{key:string;value:PlayerDetail}|null>(null),[detailLoading,setDetailLoading]=useState(false),[loading,setLoading]=useState(false),[error,setError]=useState(""),[favorites,setFavorites]=useState<number[]>(readFavorites);
  ...
  useEffect(()=>{if(!selected)return;let active=true;setDetailLoading(true);setLoadedDetail(null);setError("");void window.fcOnline.fetchPlayerDetail(selected.spId,grade,detailOptions).then(value=>{if(active)setLoadedDetail({key:detailKey,value})}).catch(reason=>{if(active)setError(reason instanceof Error?reason.message:"선수 상세 조회 실패")}).finally(()=>{if(active)setDetailLoading(false)});return()=>{active=false}},[selected,grade,detailOptions]);
  ...
  const runSearch=useCallback(async(value:string,current:PlayerSearchFilters,append=false)=>{const request=playerSearchRequest({...current,query:value},appliedSearchRef.current,append?pageRef.current+1:1);if(!request.query.trim()&&!hasFilters(request)){setError("선수명 또는 검색 조건을 입력해 주세요.");return}setLoading(true);setError("");const sequence=++searchSequenceRef.current;try{const result=await window.fcOnline.searchPlayers(request);if(sequence!==searchSequenceRef.current)return;appliedSearchRef.current=request;setRows(previous=>append?[...previous,...result.players.filter(row=>!previous.some(item=>item.spId===row.spId))]:result.players);pageRef.current=result.page;setHasMore(result.hasMore);setResultTotal(result.total);setCatalog(result.catalog);setSelected(null);if(!append){setFiltersOpen(false);requestAnimationFrame(()=>resultsRef.current?.scrollIntoView({behavior:"smooth",block:"start"}))}}catch(reason){if(sequence===searchSequenceRef.current)setError(reason instanceof Error?reason.message:"선수 검색 실패")}finally{if(sequence===searchSequenceRef.current)setLoading(false)}},[]);
